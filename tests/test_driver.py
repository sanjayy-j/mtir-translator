...
@pytest.mark.parametrize("stage", ["wat", "sbc"])
def test_unimplemented_stages_report_owner_and_week(stage, capsys):
    ...
    'cir' and 'll' were on this list until M3a and M5 landed; they now have
    the positive tests below instead.
    """
...
# -- stages that M3 has since implemented ----------------------------------
def test_emit_cir(capsys):
    assert main(["--emit=cir", str(EXAMPLE)]) == 0
    out = capsys.readouterr().out
    assert out.startswith("func @abs(i32 %x) -> i32 {")
    assert "icmp.slt i32 %x, 0" in out


def test_emit_ll(capsys):
    assert main(["--emit=ll", str(EXAMPLE)]) == 0
    out = capsys.readouterr().out
    assert "define i32 @abs(i32 %x) {" in out
    assert "br i1 %t0, label %" in out


def test_emit_ll_accepts_a_cir_file_directly(capsys):
    """The .cir format is the contract: the back end needs no front end."""
    cir = Path(__file__).parents[1] / "docs" / "examples" / "abs.cir"
    assert main(["--emit=ll", str(cir)]) == 0
    out = capsys.readouterr().out
    assert "define i32 @abs(i32 %x) {" in out
    assert "br i1 %t0, label %then, label %exit" in out


def test_opt_level_1_folds_constants(capsys):
    src = Path(__file__).parents[1] / "tests" / "corpus" / "valid" / "arith.mini"
    assert main(["--opt=1", "--emit=cir", str(src)]) == 0
    out = capsys.readouterr().out
    assert "store i32 14, %a.addr" in out       # 2 + 3 * 4
    assert "mul i32" not in out


def test_build_error_exits_1(tmp_path, capsys):
    bad = tmp_path / "bad.mini"
    bad.write_text("fn main() -> int { return nope; }")
    assert main(["--emit=cir", str(bad)]) == 1
