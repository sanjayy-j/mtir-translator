...
// ==========================================================================
// The front end is pinned to the checked-in CIR
// ==========================================================================
MTIR_TEST("builder", "the pipeline reproduces the checked-in CIR exactly") {
  // tests/cir/*.cir were generated by the Python prototype, and this test is
  // what established that the migration was a migration rather than a rewrite
  // that happened to compile: the C++ front end, semantic analysis and
  // builder produce the same text, character for character.
  //
  // The prototype is gone, so the fixtures are now simply the recorded
  // output: a change in the builder cannot slip through without showing up
  // here.  See tests/cir/README.md.
  struct Pair {
    const char *source;
    const char *fixture;
  };
